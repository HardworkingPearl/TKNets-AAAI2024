...
class Job:
    ...
    def __init__(self, train_args, sweep_output_dir):
        ...
        command = ['python', '-m', 'scripts.train_TDG_']
                ...
                for test_type in ALL_TEST_TYPE:
                    for hparams_seed in range(n_hparams_from, n_hparams):
                        ...
                        train_args['seed'] = misc.seed_hash(dataset,
                            algorithm, test_type, hparams_seed, trial_seed)
...
def read_val_args_list(path, data_dir, task):
    """ read top hps from path then concat for args
    """
    records = []
    ...
    for record in records:
        train_args = {}
        train_args['dataset'] = record['args']['dataset']
        train_args['algorithm'] = record['args']['algorithm']
        train_args['test_type'] = record['args']['test_type'].replace('test', 'val')
        train_args['holdout_fraction'] = record['args']['holdout_fraction']
        train_args['hparams_seed'] = record['args']['hparams_seed'] # not used
        train_args['data_dir'] = data_dir
        train_args['task'] = task
        train_args['trial_seed'] = record['args']['trial_seed']
        train_args['seed'] = misc.seed_hash(train_args['dataset'],
            train_args['algorithm'], train_args['test_type'], train_args['trial_seed'])
        # if steps is not None:
        #     train_args['steps'] = steps
        # if hparams is not None:
        train_args['hparams'] = record['hparams']
        args_list.append(train_args)
    return args_list
...
if __name__ == "__main__":
    ...
    parser.add_argument('--single_test_envs', action='store_true') # TODO remove
    parser.add_argument('--skip_confirmation', action='store_true')
    parser.add_argument('--val_sweep', action='store_true')
    args = parser.parse_args()

    if args.val_sweep:
        args_list = read_val_args_list(
            os.path.join(args.output_dir, 'val_records.jsonl'),
