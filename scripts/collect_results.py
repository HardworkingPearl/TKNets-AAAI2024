...
def save_top_records(records, args):
    """
    save top records into json file
    """
    records_path = os.path.join(args.input_dir, "top_records.jsonl")
    # print("save top recors to: "+records_path)
    for record in records:
        with open(records_path, 'a') as f:
            f.write(json.dumps(record['top_record'], sort_keys=True) + "\n")
...
def print_table(table, header_text, row_labels, col_labels, colwidth=10,
    ...
    if latex:
        col_labels = ["\\textbf{" + str(col_label).replace("%", "\\%").replace('_', '\_').replace('forward', 'FWD').replace('backward', 'BWD') + "}"
...
def print_results_tables(records, selection_method, latex, args):
    """Given all records, print a results table for each dataset."""
    grouped_records = reporting.get_grouped_records(records).map(lambda group:
                                                                 {**group, "sweep_acc": selection_method.sweep_acc(
                                                                     group["records"]), "top_record": selection_method.top_hparams(group["records"])}
                                                                 ).filter(lambda g: g["sweep_acc"] is not None)

    save_top_records(grouped_records, args)
    ...
    for dataset in dataset_names:
        ...
        test_types = ['forward_test', 'backward_test',
                      'forward_val', 'backward_val']

        table = [[None for _ in [*test_types, "Avg"]] for _ in alg_names]
        for i, algorithm in enumerate(alg_names):
            means = []
            for j, test_type in enumerate(test_types):
                trial_accs = (grouped_records
                              .filter_equals(
                                  "dataset, algorithm, test_type",
                                  (dataset, algorithm, test_type)
                              ).select("sweep_acc"))
...
if __name__ == "__main__":
    ...
    SELECTION_METHODS = [
        ...
        # model_selection.TDGMethod,
        model_selection.TDGMultiStepMethod
    ]
    ...
    for selection_method in SELECTION_METHODS:
        ...
        print_results_tables(records, selection_method, args.latex, args)
